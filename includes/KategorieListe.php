<?PHP
  ...
  $rezepteGrpDbTbl = new DbTable($_SESSION['config']->DBCONNECT, 'rezept_gruppen', array("id", "name",
      "beschreibung"), "", "", "name", "");
  ...
  foreach ($rezepteGrpDbTbl->ROWS as $grpRow) {
      if (getRezeptAnzahl($grpRow->getNamedAttribute("id")) > 0) {
          ...
          $r->setAttribute(1, new Text("(" . getRezeptAnzahl($grpRow->getNamedAttribute("id")) . ")", 2, false, true));

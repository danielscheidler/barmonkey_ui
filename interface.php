...
switch ( $command ) {
    ...
    case "register":
        if ( strlen( $_REQUEST['Vorname'] ) > 0 && strlen( $_REQUEST['Nachname'] ) > 0 && strlen( $_REQUEST['User'] ) >
            0 && strlen( $_REQUEST['Email'] ) > 0 && strlen( $_REQUEST['Pw'] ) > 0 ) {
    ...
    default:
        ...
        if ( !isset( $_REQUEST['User'] ) || !isset( $_REQUEST['Pw'] ) || strlen( $_REQUEST['User'] ) == 0 ||
            strlen( $_REQUEST['Pw'] ) == 0 ) {
...
function getRezeptById( $rezeptId ) {
    $rezepteTbl = new DbTable( $_SESSION['config']->DBCONNECT, 'rezepte', array( "name", "beschreibung",
        ...
        "Name, Beschreibung, Kategorie, Bild, Vorbereitung, Nachbereitung", "", "", "id = " . $rezeptId );
...
function register( $vorname, $nachname, $User, $Pw, $email ) {
    if ( strlen( $vorname ) > 0 && strlen( $nachname ) > 0 && strlen( $User ) > 0 && strlen( $Pw ) > 0 &&
        strlen( $email ) > 0 ) {

...
if ( substr( $_SERVER['DOCUMENT_ROOT'], strlen( $_SERVER['DOCUMENT_ROOT'] ) - 1, 1 ) != "/" ) {
    // FÃÂ¼hrendes Slash notfalls hinzufÃÂ¼gen
    ...
    // FÃÂ¼hrendes Slash notfalls entfernen
    if ( substr( $filename, 0, 1 ) == "/" ) {
...
if ( file_exists( $_SERVER['DOCUMENT_ROOT'] . $filename ) ) {
    $ignore = false;
    $current_agent = ( isset( $_SERVER['HTTP_USER_AGENT'] ) ) ? addslashes( trim( $_SERVER['HTTP_USER_AGENT'] ) ) :
        "no agent";
    $current_time = time();
    ...
    for ( $i = 1; $i < sizeof( $c_file ); $i++ ) {
        ...
        if ( $counter_ip == $current_ip && $current_time - $expire < $counter_time ) {
            // besucher wurde bereits gezÃÂ¤hlt, daher hier abbruch
            $ignore = true;
            break;
        }
    }

    // counter hochzÃÂ¤hlen
    if ( $ignore == false ) {
        if ( sizeof( $c_file ) == 0 ) {
            // wenn counter leer, dann fÃÂ¼llen
            $add_line1 = date( "z" ) . ":1||" . date( "W" ) . ":1||" . date( "n" ) . ":1||" . date( "Y" ) .
                ":1||1||1||" . $current_time . "\n";
            ...
            // werte zur verfÃÂ¼gung stellen
            ...
            // speichern und aufrÃÂ¤umen und anzahl der online leute bestimmten
            ...
            if ( $fp ) {
                flock( $fp, LOCK_EX );
                $add_line1 = date( "z" ) . ":" . $day . "||" . date( "W" ) . ":" . $week . "||" . date( "n" ) . ":" .
                    $month . "||" . date( "Y" ) . ":" . $year . "||" . $all . "||" . $record . "||" . $record_time . "\n";
                fwrite( $fp, $add_line1 );
        ...
        if ( sizeof( $c_file ) > 0 )
            ...
            list( $day_arr, $week_arr, $month_arr, $year_arr, $all, $record, $record_time ) = explode( "||",
                date( "z" ) . ":1||" . date( "W" ) . ":1||" . date( "n" ) . ":1||" . date( "Y" ) . ":1||1||1||" . $current_time );

...
if ( isset( $_SESSION['tmp']['gMap_GpsPosition_RowId'] ) && $_SESSION['tmp']['gMap_GpsPosition_RowId'] >
    0 ) {
    $dbTableAdressen = new DbTable( $_SESSION['config']->DBCONNECT, "gpsPositions", array( "*" ),
        // "name", "strasse", "hausnummer", "plz", "ort", "ortsteil", "email", "telefon", "handy", "fax", "icq", "homepage", "text"),
...
echo "  <script src=\"http://maps.google.com/maps?file=api&amp;v=2.x&amp;sensor=false&amp;key=" .
    getPageConfigParam( $_SESSION['config']->DBCONNECT, "google_maps_API_key" ) . "\" type=\"text/javascript\"></script>
		  <script src=\"http://www.google.com/uds/api?file=uds.js&v=1.0&key=" . getPageConfigParam( $_SESSION['config']->DBCONNECT,

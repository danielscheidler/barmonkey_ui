...
class DbConnect 
{
  ...
  function reconnect(){ //baut die verbindung auf
     $this->db_link = @mysql_pconnect($this->host,$this->user,$this->pass) or die ("Datenbankverbindung *".$this->host."|".$this->user."|".$this->pass."* nicht mÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶glich!");
  ...
  function selectDatabase($dbname){  // wÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¤hlt die datenbank
	...
	if(isset($_SESSION['config']) && isset($_SESSION['config']->CURRENTUSER) && $_SESSION['config']->CURRENTUSER->STATUS == "admin"){
		@mysql_select_db($dbname) or die ("Datenbank *" .$dbname ."* konnte nicht ausgewÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¤hlt werden!");
	} else {

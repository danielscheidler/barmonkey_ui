...
class ShortcutSidebar extends Object {
    ...
    function ShortcutSidebar( $layoutArt = "DESKTOP" ) {
        $this->SHORTCUTS_DB = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_shortcut', array( "id",
            "name", "beschreibung" ), "Id, Name, Beschreibung", "", "name", "" );
        $this->WIDTH = 140;
        $this->HEIGHT = 370;
    ...
    function getConfigName( $id ) {
        $configDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_config', array( "name" ), "",
            "", "", "id=" . $id );
    ...
    function getConfigFunkId( $id, $status ) {
        $configDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_config', array( "funk_id",
            "funk_id2", "control_art" ), "", "", "", "id=" . $id );
    ...
    function addZimmerShortcutCommandItems( $zimmerId, $onOff ) {
        ...
        $configDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_config', array( "id",
            "funk_id", "funk_id2" ), "", "", "", "zimmer=" . $zimmerId );
    ...
    function addArtShortcutCommandItems( $artId, $onOff ) {
        ...
        $configDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_config', array( "id",
            "funk_id", "funk_id2" ), "", "", "", "control_art=" . $artId );
    ...
    function addEtagenShortcutCommandItems( $etagenId, $onOff ) {
        $configDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_config', array( "id",
            "funk_id", "funk_id2" ), "", "", "", "etage=" . $etagenId );
    ...
    function addShortcutCommandItem( $id, $status ) {
        ...
        if ( !strpos( $this->SHORTCUTS_URL_COMMAND, "_" . $funkId . "-" ) && strlen( $funkId ) > 0 && strlen
            ( $status ) > 1 ) {
            ...
            $this->SHORTCUTS_TOOLTIP .= "<tr style=\"background-color:" . $this->SHORTCUTS_ROW_COLOR_LAST . ";\"><td>" .
                $this->getConfigName( $id ) . "</td><td>" . ( $status == "on" ? $this->ON_LABEL : $this->OFF_LABEL ) .
                "</td></tr>";
        }
    }
    ...
    function prepareShortcutSwitchLink( $shortcutId ) {
        ...
        $itemDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_shortcut_items', array( "id",
    ...
    function getMobileView() {
        ...
        foreach ( $this->SHORTCUTS_DB->ROWS as $shortcutRow ) {
            ...
            $cfgItemList = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_shortcut_items', array( "id",
                "shortcut_id", "config_id", "art_id", "zimmer_id", "etagen_id", "funkwahl", "on_off" ), "", "",
                "config_id DESC , zimmer_id DESC , etagen_id DESC ", "shortcut_id=" . $shortcutRow->getNamedAttribute
                ( "id" ) );
            ...
            foreach ( $cfgItemList->ROWS as $cfgItm ) {
                ...
                if ( strlen( $cfgItm->getNamedAttribute( "config_id" ) ) > 0 ) {
                    ...
                    $configDb = new DbTable( $_SESSION['config']->DBCONNECT, 'homecontrol_config', array( "id", "name",
                        "funk_id", "funk_id2" ), "", "", "", $whereStmt );

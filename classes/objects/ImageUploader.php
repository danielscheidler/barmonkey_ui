...
class ImageUploader extends Object {
    ...
    function ImageUploader( $targetPath, $filePrefix = "", $dbInsertTable = "", $dbInsertColumn = "", $dbInsertId =
        "", $hiddenObj = "", $targetFileName = "" ) {
    ...
    function insertIntoDbIfNeeded( $newFileName ) {
        ...
        if ( strlen( $this->DB_INSERT_TABLE ) > 0 && strlen( $this->DB_INSERT_COLUMN ) > 0 && strlen( $this->DB_INSERT_ID ) >
            0 ) {

            $sql = "UPDATE " . $this->DB_INSERT_TABLE . "   SET " . $this->DB_INSERT_COLUMN . " = '" . $newFileName .
    ...
    function showFileUploadMask() {
        ...
        $btn = new Button( "upload", "Datei hochladen",
            "alert('Datei wird hochgeladen.Dieser Vorgang kann einige Zeit in Anspruch nehmen!Nach Abschluss erhalten Sie eine BestÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¤tigung des Upload-Vorgangs.Brechen Sie nicht vorher Ab!')" );
    ...
    function doUploadFile() {
        ...
        $newFileName = $folder->doUploadFile( $_FILES['probe'], substr( $folder->getPath(), strlen( $_SERVER['DOCUMENT_ROOT'] ) ),
            $this->getFilePrefix(), $this->TARGETFILENAME );
     ...
     */
    function show() {
        if ( !isset( $_REQUEST['picImportUpload'] ) || ( isset( $_REQUEST['picImportUpload'] ) && $_REQUEST['picImportUpload'] !=
            "doUpload" ) ) {

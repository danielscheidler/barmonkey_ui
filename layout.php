...
$_SESSION['mainpanelwidth'] = 600;
	...
	$layoutTable->addSpacer(1,0);	
       ...
       $headerLayoutRow->setAttribute(0, new Title("Barmonkey",0,6));
    ...
    /* ------------------------------------
      HAUPTPANEL
    ------------------------------------ */ 
       $GrpPanel = new DivByInclude(getRunlinkParam($_SESSION['config']->DBCONNECT, "rezeptgruppen", "link"), false);
       $GrpPanel->setBackgroundColor( $_SESSION['config']->COLORS['panel_background']);
       $GrpPanel->setBorder(0);
       $GrpPanel->setOverflow("suto");
       $GrpPanel->setWidth(220);
    ...
    $line = new Line();
    $t = new Text("Arduino URL: ".$_SESSION['config']->PUBLICVARS['arduino_url'] ."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", 2, false, true, false, false);
    $t2 = new Text("( " .$_SESSION['aktiveRezeptAnzahl'] ." von insgesamt " .$_SESSION['rezeptAnzahl'] ." Rezepten. )", 2, false, true, false, false);

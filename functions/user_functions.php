...
function getUserImageByPicname($picName, $width = 200) {
    ...
    if (strlen($picName) > 0 && !file_exists($_SERVER['DOCUMENT_ROOT'] . substr(dirname
        ($_SERVER['SCRIPT_NAME']), 1) . "/pics/user/" . $picName)) {
...
function getUserImagePath($userId, $trimLeftCount = 0) {
    ...
    if (strlen($picName) > 0 && !file_exists($_SERVER['DOCUMENT_ROOT'] . substr(dirname
        (substr($_SERVER['SCRIPT_NAME'], $trimLeftCount)), 1) . "/pics/user/" . $picName)) {
        $pic = substr(dirname($_SERVER['SCRIPT_NAME']), 1) . "/pics/unknownpic.jpg";
    } else {
        if (strlen($picName) == 0) {
            $pic = substr(dirname($_SERVER['SCRIPT_NAME']), 1) . "/pics/unknown_user.jpg";
...
function isUserOnFriendlist($friendId) {
    $sql = " SELECT * FROM freundesliste " . " WHERE user_id = " . $_SESSION['config']->
        CURRENTUSER->USERID . " AND friend_id = " . $friendId;
...
function getUserImageSourceByPicname($picName, $width = 200) {
    ...
    if (strlen($picName) > 0 && !file_exists($_SERVER['DOCUMENT_ROOT'] . $sn .
        "pics/user/" . $picName)) {
...
function getUserprofilAsTooltipText($userId) {
    ...
    $ret = "<table width='300'>" . "<tr><th colspan='2'><font size='2' color='" . $color .
        "'>" . $retRow['User'] . "</font></th></tr>" . "<tr><th colspan='2'>" .
        getUserImageSourceByPicname($retRow['pic'], 150) . "</th></tr>" .
        "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Name: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Vorname'] .
        " " . $retRow['Nachname'] . "</font></td></tr>" .
        "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Status: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Status'] .
        "</font></td></tr>" . "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Strasse: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Strasse'] .
        "</font></td></tr>" . "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Plz/Ort: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Plz'] .
        " " . $retRow['Ort'] . "</font></td></tr>" .
        "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Telefon: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Telefon'] .
        "</font></td></tr>" . "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Fax: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Fax'] .
        "</font></td></tr>" . "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Handy: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Handy'] .
        "</font></td></tr>" . "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>ICQ: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Icq'] .
        "</font></td></tr>" . "<tr><td width = '20%'><font size='2' color='" . $color .
        "'>Homepage: </font></td><td width = '80%'><font size='2' color='#555555'>" . $retRow['Homepage'] .
        "</font></td></tr>" . "</table>";
...
function isLoggedIn() {
    return $_SESSION['config']->CURRENTUSER->STATUS == "admin" || $_SESSION['config']->
        CURRENTUSER->STATUS == "user";
}


function isLastGpsUpdateInLast5Minutes($dbConnect, $userid) {
    $sql = "SELECT (now() - timeonupdate) as diffTime FROM  gpsPositions WHERE user_id = " .
        $userid;
